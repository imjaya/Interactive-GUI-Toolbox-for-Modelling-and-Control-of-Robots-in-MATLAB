...
% Last Modified by GUIDE v2.5 03-Dec-2019 21:19:08
...
function pushbutton1_Callback(hObject, eventdata, handles)
    ...
    r1=evalin('base','r1');
    DH=evalin('base','DH');
    finaljacob = Geometric_Jacob_Sym(DH);
    userangles = str2double(get(handles.inputjacobang,'data'));
    if r1==1
        fj = arrayfun(@char, finaljacob, 'uniform', 0);
        set(handles.tab1, 'data',fj);
    else
        jnum=R.jacobn(userangles'*(pi/180));
        set(handles.tab1, 'data',jnum);
    end
...
function pushbutton2_Callback(hObject, eventdata, handles)
    DH=evalin('base','DH');
    [singular_eq,singular_joint_val]= singularity(DH);
    set(handles.tab3, 'data',singular_joint_val*(180/pi));
    eq = arrayfun(@char,singular_eq, 'uniform', 0);
    set(handles.singtext, 'String',eq);
